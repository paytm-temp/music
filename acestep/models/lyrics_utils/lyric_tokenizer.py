...
# Add to the _abbreviations dictionary
_abbreviations["hi"] = [
    (re.compile("\\b%s\\." % x[0], re.IGNORECASE), x[1])
    for x in [
        ("dr", "doctor"),
...
# Add to the _symbols_multilingual dictionary
_symbols_multilingual["hi"] = [
    (re.compile(r"%s" % re.escape(x[0]), re.IGNORECASE), x[1])
    for x in [
        ("&", " and "),
        ("@", " at "),
        ...
        ("=", " equal "),
        ("~", " approximately "),
        # Add more Hinglish-specific symbols
    ]
]

# Add Hinglish abbreviations
_abbreviations["hi"] = [
    (re.compile("\\b%s\\." % x[0], re.IGNORECASE), x[1])
    for x in [
        ("dr", "doctor"),
        ...
        ("sec", "second"),
        ("hrs", "hours"),
        # Add more Hinglish abbreviations
    ]
]
...
_currency_re = {
    ...
    "INR": re.compile(r"((₹[0-9\.\,]*[0-9]+)|([0-9\.\,]*[0-9]+₹))"),  # Add INR support
}
...
def expand_numbers_multilingual(text, lang="en"):
    if lang == "zh":
        ...
        try:
            ...
            text = re.sub(
                _currency_re["INR"], lambda m: _expand_currency(m, lang, "INR"), text
            )
        except:
            pass
        if lang != "tr":
...
class VoiceBpeTokenizer:
    def __init__(self, vocab_file=DEFAULT_VOCAB_FILE):
        ...
        self.char_limits = {
            ...
            "hu": 224,
            "ko": 95,
            "hi": 500  # Increased limit for Hinglish to handle code-mixing
        }
...
def test_expand_numbers_multilingual():
    test_cases = [
        ...
        # Add Hinglish test cases
        ("Main 1st floor pe rehta hun", "Main pehla floor pe rehta hun", "hi"),
        ("Mujhe 2 cup chai chahiye", "Mujhe do cup chai chahiye", "hi"),
        ("₹1000 ka bill hai", "ek hazaar rupaye ka bill hai", "hi"),
        ("Score 99.5% hai", "score ninety nine point five percent hai", "hi"),
    ]
